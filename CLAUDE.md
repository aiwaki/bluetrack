...
- `host/macos-hid-inspector/`: SwiftPM tool. `scan` and `watch` use IOHID to see
  ...
  correction packets via CoreBluetooth, mirroring the Python sender. `selftest`
  round-trips `FeedbackCrypto` without touching Bluetooth (useful when only
  CommandLineTools is installed). The `BluetrackHostKit` library target holds
  the shared crypto contract.
