...
/**
 * Cascading fade + lift entry animation for the Diagnostics
 * route cards. Each card calls this with a unique `index`; the
 * effect launches once per composition lifetime with a delay of
 * `index * 60ms`, then tweens `alpha 0 → 1` and `translationY
 * (offsetDp) → 0` over 320ms with `FastOutSlowInEasing`.
 *
