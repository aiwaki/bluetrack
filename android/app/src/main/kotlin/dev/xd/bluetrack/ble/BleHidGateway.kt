...
class BleHidGateway(private val context: Context, private val engine: TranslationEngine) {
    ...
    fun initialize() {
        val bluetoothAdapter = adapter ?: run {
            _state.value = "Bluetooth Unavailable"
            return
        }
        ...
        try {
            bluetoothAdapter.getProfileProxy(context, object : BluetoothProfile.ServiceListener {
                ...
                override fun onServiceDisconnected(profile: Int) {
                    ...
                    host = null
                    registeredMode = null
                    _state.value = "HID Disconnected"
                }
            }, BluetoothProfile.HID_DEVICE)
            startGatt()
        } catch (_: SecurityException) {
            _state.value = "Bluetooth Permission Missing"
        }
    }
        ...
        try {
            ...
            val accepted = device.registerApp(sdp, null, q, ioExecutor, object : BluetoothHidDevice.Callback() {
                override fun onAppStatusChanged(pluggedDevice: BluetoothDevice?, registered: Boolean) {
                    registeredMode = if (registered) pendingMode else null
                    _state.value = if (registered) "HID Ready (${pendingMode.name})" else "HID Registration Failed"
                }
            ...
            if (!accepted) _state.value = "HID Registration Rejected"
        } catch (_: SecurityException) {
            _state.value = "Bluetooth Permission Missing"
        }
    }
    ...
    private fun startGatt() {
        ...
        val service = BluetoothGattService(
            UUID.fromString("0d03f2a3-b9b2-43f6-90ca-6c4ff67c2263"),
            BluetoothGattService.SERVICE_TYPE_PRIMARY,
        )
        service.addCharacteristic(
            BluetoothGattCharacteristic(
                UUID.fromString("4846ff87-f2d4-4df2-9500-9bf8ed23f9e6"),
